...
model {
  ...
  // priors
  for(k in 1:K){
    //target += gamma_lpdf(lambda[k] | a_0_lambda, b_0_lambda) + 0.5*log(10);
    target += gamma_lpdf(tilde_p[k]/(1-tilde_p[k]) | a_0_lambda, b_0_lambda) - 2*log(1-tilde_p[k]);
    //target += gamma_lpdf(phim1[k] | a_0_phi, b_0_phi) + 0.5*log(10);
    target += gamma_lpdf(tilde_phim1[k]/(1-tilde_phim1[k]) | a_0_phi, b_0_phi) - 2*log(1-tilde_phim1[k]);
    //target += inv_gamma_lpdf(tilde_phim1[k]/(1-tilde_phim1[k]) | a_0_phi, b_0_phi) - 2*log(1-tilde_phim1[k]);
    ...
    //target += student_t_lpdf(beta_0[k] | 4,  beta_0_loc[k], beta_0_scale);
    ...
    //target += exponential_lpdf(exp(tilde_log_ray[k])*sqrt(D*D*P) | r_alpha) + 0.5*log(D*D*P) + tilde_log_ray[k];
    
  }
  // likelihood
  target += llk_lp(N, K, D, P, m, y, beta_0, theta, tau,  
